    ...
    <div className="min-h-[calc(100vh-4rem)] py-12 bg-gradient-to-br from-handloom-50 via-white to-gold-50 flex items-center justify-center">
      ...
      >
        <div className="text-center mb-8">
          <h1 className="text-3xl font-playfair font-bold text-handloom-900 mb-2">{t('auth.registerTitle')}</h1>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-handloom-700 mb-2">{t('auth.fullName')}</label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-handloom-400 w-5 h-5" />
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full pl-11 pr-4 py-3 border border-handloom-200 rounded-xl focus:ring-2 focus:ring-handloom-500" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-handloom-700 mb-2">{t('auth.email')}</label>
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 text-handloom-400 w-5 h-5" />
        ...
        <p className="text-center text-sm text-handloom-600 mt-6">
          {t('auth.haveAccount')} <Link to="/login" className="font-semibold text-handloom-700 hover:underline">{t('auth.signIn')}</Link>
        </p>
      </motion.div>
    </div>
  );
